...
test.describe("Team Draft 4v4 (8 players, 3 rounds, Swiss cross-team)", () => {
  ...
  test("complete 4v4 lifecycle: create friday -> pods -> rounds -> standings", async ({
    ...
    // -----------------------------------------------------------------
    // 5. Create 8 phony users (auto-RSVPed)
    // -----------------------------------------------------------------
    ...
    for (let roundNum = 1; roundNum <= 3; roundNum++) {
      ...
      const resultPatterns = [
        { p1Wins: 2, p2Wins: 1, draws: 0 }, // match win for p1
        { p1Wins: 1, p2Wins: 2, draws: 0 }, // match win for p2
        { p1Wins: 2, p2Wins: 0, draws: 0 }, // 2-0 for p1
        { p1Wins: 0, p2Wins: 2, draws: 0 }, // 2-0 for p2
      ];
    ...
    // With Swiss cross-team, no rematches are expected (4v4 x 3 rounds
    // can pair each A with 3 of 4 Bs, so 12 unique pairings out of 16 possible)
