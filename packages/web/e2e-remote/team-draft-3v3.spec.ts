...
// ---------------------------------------------------------------------------
// Team Draft 3v3 — full lifecycle
// ---------------------------------------------------------------------------

test.describe("Team Draft 3v3 (6 players, 3 rounds, Latin square)", () => {
  test.setTimeout(60_000);

  test("complete 3v3 lifecycle: create friday -> pods -> rounds -> standings", async ({
    request,
  }) => {
    // -----------------------------------------------------------------
    ...
    const cubeSlug = `test-3v3-${Date.now()}`;
    ...
    expect(podDetail.pod.format).toBe("team_draft_3v3");
    expect(podDetail.seats.length).toBe(6);
    ...
    // Track all pairings across rounds to verify Latin square
    ...
    for (let roundNum = 1; roundNum <= 3; roundNum++) {
      ...
      for (const match of roundMatches) {
        ...
        // Track pairing to verify no duplicates across rounds
        const key = [match.player1Id, match.player2Id].sort().join("-");
        expect(allPairKeys.has(key)).toBe(false);
        allPairKeys.add(key);
      }
    ...
    expect(standingUserIds.size).toBe(6);
  });
});
