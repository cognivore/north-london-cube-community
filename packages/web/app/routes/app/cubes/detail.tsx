...
export async function loader({ request, params }: { request: Request; params: { cubeId: string } }) {
  ...
  const [cubesResult, meResult] = await Promise.all([
    api.listCubes(ch),
    api.me(ch),
  ]);
  if (!cubesResult.ok) throw new Response("Failed", { status: 500 });
  const cube = cubesResult.data.cubes.find((c: any) => c.id === params.cubeId);
  if (!cube) throw new Response("Not found", { status: 404 });
  const isOwner = meResult.ok && meResult.data.user.id === cube.ownerId;
  return { cube, isOwner };
}
...
export default function CubeDetail() {
  const { cube, isOwner } = useLoaderData<typeof loader>();
  const actionData = useActionData<typeof action>();

  if (!isOwner) return <CubeReadOnly cube={cube} />;
...
function CubeReadOnly({ cube }: { cube: any }) {
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-ink">{cube.name}</h1>
      ...
      <div className="grid grid-cols-2 gap-4 text-sm">
        <div className="rounded-sm bg-paper-alt p-3">
          <p className="text-xs text-ink-faint">Card count</p>
          <p className="text-lg font-semibold text-ink mono" data-mono>{cube.cardCount}</p>
        </div>
        <div className="rounded-sm bg-paper-alt p-3">
          <p className="text-xs text-ink-faint">Pod size</p>
          <p className="text-lg font-semibold text-ink mono" data-mono>{cube.minPodSize}–{cube.maxPodSize}</p>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="mt-1 flex flex-wrap gap-2">
          {cube.supportedFormats?.map((f: string) => (
            <span key={f} className="rounded-sm bg-paper-alt px-2 py-1 text-xs text-ink-soft">
              {f.replace(/_/g, " ")}
            </span>
          ))}
        </div>
      ...
      {cube.retired && <p className="text-sm text-warn">This cube is retired.</p>}
    </div>
  );
}
